...
[仓库](https://github.com/xudaniel/ai-news-agent) · [配置步骤](docs/setup-zh.md) · [选题与写作要求](docs/editorial-policy.md) · [测试与限制](docs/verification.md) · [手机邮件版式示例](docs/email-preview.html)

## 阅读格式

- 最多五条，按实际影响排序；不足五条如实说明。
- 优先 AI，重大芯片、云计算、机器人、网络安全及科技商业事件也可入选。
- 先读「30 秒速览」：一句今日判断与五条中文标题。
- 每条显示「投资／产品／监管」和「已上线／测试中／已宣布／政策提案」等中文状态。
- 正文保留日期、事实、影响推断、观察点与原始来源；有历史依据时明确新增变化。
- 结尾只给一个「今日一个行动」，对应最优先的新闻。
- 同时生成 `news.md`、手机 HTML 预览 `news.html` 与固定两页打印版 `news-print.html`：白底黑字、黑色来源链接、欣远景投资灰阶横版标识。打印版第一页放速览和前两条，第二页放其余新闻、横向判断与今日行动。
- 每条显示重要性、影响周期、谁受益／谁承压和后续验证指标；顶部判断附本期事实依据。
- 新闻和阅读标签全中文，产品名称可保留官方写法。

仓库的模型路径使用 RSS 标题和摘要，并不自动打开每篇全文核验，因此正文会明确说明这个证据边界。现有定时助手任务另外进行网页检索和来源核验，也负责查找过去七天的邮件以避免跨日重复。**仓库支持展示经审核的跨期新增字段，但自身没有跨日语义新闻记忆；七天历史检索由现有 Gmail 任务负责。**
